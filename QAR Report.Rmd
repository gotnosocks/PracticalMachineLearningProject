...
Cleaning the data in this way removed all the NA, NaN, and Divide by Zero entries from the dataset, thus there was not need to handle them otherwise.
...
For this analysis, I chose to include all variables. I was more interested in the performance of the different models than in feature selection. Additionally, I suspected that I would use the random forest classifier in the end, and it is insensitive to the addition of correlated features. 
...
The out of sample error rate for the random forest classifier is
```{r, echo=FALSE}
1-as.numeric(confusionMatrix(pred.rf, testing$classe)$overall[1])
```
This out of sample error rate was calculated using 30% of the training data, which was withheld when the model was trained. This data subset is referred to as "testing" in the R code included in this document.
